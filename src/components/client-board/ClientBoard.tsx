        ...
        <div className='fixed top-20 left-0'>
            <div className="relative p-[4px] min-w-[200px] w-fit rounded-md bg-gray-800 overflow-hidden">
                ...
                <div className="relative bg-gray-800 rounded-md p-4 z-10">
                    {playerInfo ? null : <h2 className="font-semibold text-xl text-white">Player Name</h2>}
                    <h2 className="font-semibold text-xl text-white">{playerInfo?.name}</h2>
                    {playerInfo ? <h3 className="text-white">Points: <span className="font-medium">{playerInfo?.totalPoints}</span></h3> : null}
