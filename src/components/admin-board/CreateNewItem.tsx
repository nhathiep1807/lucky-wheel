...
function CreateNewItem({ isOpen, handleIsOpen }: Props) {
  ...
  const { mutate: creacreateWheelItem, isPending: isPendingCreateNewItem } =
    useCreateWheelItemMutation();

  const { handleSubmit, register, formState, reset } =
    useForm<CreateWheelItemsForm>({
      defaultValues: {
        name: "",
        value: "",
        categoryId: "1",
      ...
      <Dialog
        ...
        actionButton={
          <div className="flex items-center gap-2 pt-4">
            <Button
              name="Cancel"
              onClick={handleClickCancelCreateNewItem}
            ></Button>
            <Button
              name="Create"
              onClick={handleSubmit(onSubmit)}
              isLoading={isPendingCreateNewItem}
            ></Button>
          </div>
        }
      >
