...
import { QueueListIcon, XCircleIcon, PencilSquareIcon } from '@heroicons/react/24/outline'
...
function ListItem({ handleUpdateWheelItem }: Props) {
    ...
    return (
        <div>
            <div className='flex items-center gap-1 py-2'><QueueListIcon className='w-5 h-5' /><span className='font-semibold'>List Item</span></div>
                ...
                {isLoading ?
                    ...
                    <div>
                        {listWheelItems?.data.map((item, _index) => (
                            <div key={item.id} className='m-4 pb-1 border-b flex justify-between'>
